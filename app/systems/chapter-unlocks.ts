import {migrateInvestigations} from '../investigations/state.ts';
...
export function migrateChapterUnlocks(s:GameState,catalog=chapterCatalog){
 if(s.investigations||s.campaign.completedChapters.includes('chapter-4')||s.campaign.chapters['chapter-4']?.completed)migrateInvestigations(s);
