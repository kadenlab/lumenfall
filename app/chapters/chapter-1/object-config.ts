...
export function placeObjects(ctx:ChapterContext){if(ctx.sceneryOnly)return;const s=view(ctx.state);for(const p of placements[s.area]??[]){if(p.kind==='enemy'&&s.wins.includes(p.id))continue;const mesh=p.sprite?ctx.sprite(p.sprite,p.x,p.z,p.scale):undefined;const o=ctx.addObj(p.kind,p.name,p.x,p.z,p.id);if(mesh)o.mesh=mesh}for(const [id,c] of Object.entries(treasures)){if(c.area!==s.area)continue;ctx.chest(c.x,c.z,id);ctx.objects[ctx.objects.length-1].name=s.chests.includes(id)?'空の宝箱':c.name}}
