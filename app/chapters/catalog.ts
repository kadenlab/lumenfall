...
export type DebugDefinition={names:string[];apply:(s:GameState,name:string)=>number;mapGates:Record<number,string>;placements:()=>Promise<{placements:Record<number,any[]>}>};
...
export function prerequisite(id:string,catalog=chapterCatalog){return id==='trials'?'chapter-2':catalog.find(c=>c.id===id)?.requires}
