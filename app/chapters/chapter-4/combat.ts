...
export function enemyAttack(c:ChapterContext,id:string,turn:number,cmd:string,_base:number){fight.phase=c.battle?.phase??1;if(id.startsWith('ray')&&cmd!=='fire')c.state.mp=Math.max(0,c.state.mp-(resists(c.state)?2:4));if(id==='ignas'&&c.battle){c.battle.hp=Math.min(c.battle.max,c.battle.hp+recovery(fight,turn));for(const l of ['attack','defense','cycle'] as Lamp[]){const m=c.world.userData.ringLamps?.[l];if(m)m.emissiveIntensity=active(fight,l,turn+1)?2.1:.08}}if(id==='seawarden'){const m=c.world.userData.signal;if(m){m.color.set((turn+1)%4===0?'#ffffff':'#76d5d3');m.emissive.set((turn+1)%4===0?'#ffffff':'#76d5d3');m.emissiveIntensity=(turn+1)%4===0?3.5:1.2}}return retaliation(fight,id,turn)}
