import type {InvestigationState} from '../investigations/state.ts';
...
export type GameState={investigations?:InvestigationState;gear?:GearState;trials:TrialState;area:number;x:number;z:number;hp:number;mp:number;lv:number;xp:number;gold:number;potions:number;ethers:number;sword:boolean;armor:boolean;time:number;campaign:Campaign};
