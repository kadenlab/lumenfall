...
Chapter 1（町→琥珀の丘→星眠りの遺跡→ヴェルグ→クリア）の内容を独立したコンテンツパックに移しました。Chapter 2はchapter-2に追加済みです。詳細はCHAPTER-2.mdを参照してください。描画、入力、戦闘、プレイヤー、インベントリ、音声再生、UI枠は共通です。
...
以下はChapter 2追加時の手順です。次の章ではchapter-2をchapter-3へ読み替え、必要な固有コンテンツを実装してください。
