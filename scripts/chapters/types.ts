...
export type Chapter={
 combat?:{start?:()=>void;onPhase?:(ctx:ChapterContext)=>void;enemyLabel?:(ctx:ChapterContext,id:string,turn:number)=>string;damage:(ctx:ChapterContext,id:string,turn:number,command:string,base:number)=>number;enemyAttack:(ctx:ChapterContext,id:string,turn:number,command:string,base:number)=>number;status:(ctx:ChapterContext,id:string,turn:number)=>string;onCommand?:(ctx:ChapterContext,id:string,turn:number,command:string)=>void};
 ...
 music:{field:number[];night:number[];battle:number[];phase?:number[]};effects:{phaseFog:string;phaseDensity:number;phaseSun:string};
