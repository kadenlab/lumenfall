...
export function decode(raw:string):GameState{
 ...
 migrateTrials(s);migrateChapterUnlocks(s);s.lv=Math.max(1,Math.min(999,Math.floor(s.lv)));s.hp=Math.max(1,Math.min(maxHealth(s),s.hp));s.mp=Math.max(0,Math.min(30+(s.lv-1)*8,s.mp));
